        ...
        var data = google.visualization.arrayToDataTable([
          <?php
            ...
            $nbJours = $nbJoursTimestamp/86400;
             if ($nbJours > 30)
              $query='SELECT monthname(`ticket_request`.resolution_date) as mois,AVG(TIMESTAMPDIFF(HOUR,`ticket`.start_date,`ticket_request`.resolution_date)) as total FROM `ticket`,`ticket_request` where `ticket_request`.resolution_date is not null and `ticket_request`.resolution_date BETWEEN "'.$date_min.'" and "'.$date_max.'" and `ticket_request`.id=`ticket`.id group by month(`ticket_request`.resolution_date),year(`ticket_request`.resolution_date) order by date(`ticket_request`.resolution_date)';
             else
              $query='SELECT concat(day(`ticket_request`.resolution_date),\'/\',month(`ticket_request`.resolution_date)) as mois,AVG(TIMESTAMPDIFF(HOUR,`ticket`.start_date,`ticket_request`.resolution_date)) as total FROM `ticket`,`ticket_request` where `ticket_request`.resolution_date is not null and `ticket_request`.resolution_date BETWEEN "'.$date_min.'" and "'.$date_max.'" and `ticket_request`.id=`ticket`.id group by date(`ticket_request`.resolution_date) order by date(`ticket_request`.resolution_date)';
