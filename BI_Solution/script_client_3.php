      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          <?php
              ...
              $query='SELECT `priv_user`.login as user , AVG(TIMESTAMPDIFF(HOUR,`ticket`.start_date,`ticket_request`.assignment_date)) as total FROM `ticket`,`ticket_request`,`priv_user` where `ticket_request`.assignment_date is not null and `ticket_request`.assignment_date BETWEEN "'.$date_min.'" and "'.$date_max.'" and `ticket_request`.id = `ticket`.id and `ticket`.agent_id = `priv_user`.contactid group by `priv_user`.login ';
                    ...
                    $pie_chart_data = array();
                      foreach($results as $result)
                          {$pie_chart_data[] = array($result["user"],(int)$result["total"]);}
                            ...
                            echo "['Agent','Durée moyenne d\'assignation'],";
